...
def get_cusres(country, df_ww_cusres):
    df_ww_cusres = df_ww_cusres[df_ww_cusres['Country of Destinatn'].isin([
                                                                          country])]
    df_ww_cusres = df_ww_cusres.convert_dtypes()
    df_ww_cusres['HP Product Number'].fillna('Blank', inplace=True)
    df_ww_cusres['HP Product Number'] = df_ww_cusres['HP Product Number'].apply(
        lambda x: x[:x.rfind('#')] if x.rfind('#') != -1 else x)
    df_ww_cusres['HP Product Number'] = df_ww_cusres['HP Product Number'].apply(
        lambda x: x[:x.rfind(' ')] if x.rfind(' ') != -1 else x)
    df_ww_cusres['IC Invoice Number'] = df_ww_cusres['IC Invoice Number'].astype('string')
    df_ww_cusres['key'] = df_ww_cusres['IC Invoice Number'] + \
        df_ww_cusres['HP Product Number']
    return df_ww_cusres
...
def compare(df_final, df_duty):
    ...
    df_final['CURRENCY'] = df_final['CURRENCY'].apply(
        lambda x: 'USD' if x == 'US Dollar' else x)
    ...
    df_final.loc[:, 'LINE_AMOUNT':] = df_final.loc[:,
                                                   'LINE_AMOUNT':].apply(pd.to_numeric, errors='coerce')
    df_final.loc[:, :'HTS assigned'] = df_final.loc[:,
                                                    :'HTS assigned'].astype('string')
    df_final['Duty Rate'] = df_final['Duty Rate'].apply(lambda x: x/100)
    df_final['Import VAT Tax Rate'] = df_final['Import VAT Tax Rate'].apply(
        lambda x: x/100)
    df_final['VALIDATION DUTY'] = (
        df_final['LINE_AMOUNT'] + df_final['Harbor Maintence Fee (Amount)']) * df_final['DUTY']
    df_final['VALIDATION VAT'] = ((df_final['VALIDATION DUTY'] + df_final['LINE_AMOUNT'] +
                                  df_final['Harbor Maintence Fee (Amount)']) * df_final['VAT']) * df_final['Exchange Rate (KF)']
    df_final['TOTAL_INV_AMT'] = df_final['TOTAL_INV_AMT'] * df_final['Exchange Rate (KF)']
    ...
    # totalizar 'Invoice total value'
    df_total_invoice = df_final.groupby(['IC Invoice Number'])[
        ['Invoice total value']].sum()
    df_total_invoice.reset_index(inplace=True)
    df_final.drop(columns=['Invoice total value'], inplace=True)
    df_final = df_final.merge(
        df_total_invoice, on='IC Invoice Number', how='left')

    df_final['COMPARE_PRODUCT_NUMBER'] = df_final.apply(
        lambda x: compare_columns(x['PRODUCT_NO'], x['HP Product Number']), axis=1)
    df_final['COMPARE_COUNTRY_ORIGIN'] = df_final.apply(lambda x: compare_columns(
        x['COO'], x["Product's Country of Origin (CIC) in ISO Code"]), axis=1)
    df_final['COMPARE_INCOTERMS'] = df_final.apply(
        lambda x: compare_columns(x['TERMS_OF_DELIVERY'], x['Incoterms']), axis=1)
    df_final['COMPARE_CURRENCY'] = df_final.apply(
        lambda x: compare_columns(x['CURRENCY'], x['Entry Value Currency']), axis=1)
    df_final['COMPARE_HTS'] = df_final.apply(
        lambda x: compare_columns(x['HTS'], x['HTS assigned']), axis=1)
    df_final['COMPARE_DUTY_RATE'] = df_final.apply(
        lambda x: compare_columns(x['DUTY'], x['Duty Rate']), axis=1)
    df_final['COMPARE_DUTY'] = df_final.apply(lambda x: compare_columns2(x['VALIDATION DUTY'], x['Duty Charge Amount']) if x['VALIDATION DUTY']
                                              != 'Blank' and x['Duty Charge Amount'] != 'Blank' else compare_columns(x['VALIDATION DUTY'], x['Duty Charge Amount']), axis=1)
    df_final['COMPARE_LINE_AMOUNT'] = df_final.apply(
        lambda x: compare_columns2(x['LINE_AMOUNT'], x['Item Total Amount']), axis=1)
    df_final['COMPARE_LINE_QTY'] = df_final.apply(
        lambda x: compare_columns(x['LINE_QTY'], x['Qty in Doc. UoM']), axis=1)
    df_final['COMPARE_VAT_RATE'] = df_final.apply(
        lambda x: compare_columns(x['VAT'], x['Import VAT Tax Rate']), axis=1)
    df_final['COMPARE_VAT'] = df_final.apply(lambda x: compare_columns2(x['VALIDATION VAT'], x['Import VAT Tax amount']) if x['VALIDATION VAT']
                                             != 'Blank' and x['Import VAT Tax amount'] != 'Blank' else compare_columns(x['VALIDATION VAT'], x['Import VAT Tax amount']), axis=1)
    df_final['COMPARE_UNIT_PRICE'] = df_final.apply(
        lambda x: compare_columns2(x['UNIT_PRICE'], x['Unit Price']), axis=1)
    df_final['COMPARE_TOTAL_INV_AMT'] = df_final.apply(
        lambda x: compare_columns2(x['TOTAL_INV_AMT'], x['Invoice total value']), axis=1)
    df_final['COMPARE_HTS_CLASS'] = df_final.apply(lambda x: compare_columns4(x['HTS'], x['HTS Class']), axis=1)

    df_final.rename(columns={'Entry Number': 'ENTRY_NUMBER', 'INVOICE_NUMBER': 'INVOICE_NUMBER_GTS', 'IC Invoice Number': 'INVOICE_NUMBER_CUSRES', 'PRODUCT_NO': 'PRODUCT_NUMBER_GTS', 'HP Product Number': 'PRODUCT_NUMBER_CUSRES', 'End Usr Name Add 1': 'END_USER', 'Entry Date': 'ENTRY_DATE', 'COO': 'COUNTRY_ORIGIN_GTS', "Product's Country of Origin (CIC) in ISO Code": 'COUNTRY_ORIGIN_CUSRES', 'TERMS_OF_DELIVERY': 'INCOTERMS_GTS', 'Incoterms': 'INCOTERMS_CUSRES', 'CURRENCY': 'CURRENCY_GTS', 'Entry Value Currency': 'CURRENCY_CUSRES', 'HTS': 'HTS_GTS', 'HTS assigned': 'HTS_CUSRES', 'VALIDATION DUTY': 'DUTY_GTS', 'Duty Charge Amount': 'DUTY_CUSRES',
                    'VALIDATION VAT': 'VAT_GTS', 'Import VAT Tax amount': 'VAT_CUSRES', 'LINE_AMOUNT': 'LINE_AMOUNT_GTS', 'Item Total Amount': 'LINE_AMOUNT_CUSRES', 'LINE_QTY': 'LINE_QTY_GTS', 'Qty in Doc. UoM': 'LINE_QTY_CUSRES', 'VAT': 'VAT_RATE_GTS', 'Import VAT Tax Rate': 'VAT_RATE_CUSRES', 'UNIT_PRICE': 'UNIT_PRICE_GTS', 'Unit Price': 'UNIT_PRICE_CUSRES', 'DUTY': 'DUTY_RATE_GTS', 'Duty Rate': 'DUTY_RATE_CUSRES', 'TOTAL_INV_AMT': 'TOTAL_INV_AMT_GTS', 'Invoice total value': 'TOTAL_INV_AMT_CUSRES', 'Total statistical vl': 'TOTAL_STATISTICAL_CUSRES', 'Harbor Maintence Fee (Amount)': 'HARBOR MAINTENCE FEE (AMOUNT)'}, inplace=True)
...
def send_email(list_criterio, list_qty_equal, list_qty_different, list_qty_blanks_cusres, week, range_date, total_rows, file_absolute_report, file_absolute_img):
    ...
    message.To = "ana-maria.campos@hp.com; rafael.patino@hp.com; karla.botello.tirado@hp.com"
    #message.To = "arnol.plazas@hp.com"
    message.Subject = 'Discrepancias Declaraciones vs Documentos HP Semana ' + \
        week + ' ' + range_date
...
def get_cusres_qty(df_ww_cusres, df_gts, country):
    ...
    df_dmr['DESTINATION COUNTRY'] = df_dmr['DESTINATION COUNTRY'].str.replace('Mexico', country)
...
def run():
    ...
    df_duty = get_db_additional('MX')

    df_ww_cusres_qty = get_cusres('MX', df_ww_cusres_qty)
    df_ww_cusres_dq = get_cusres('MX', df_ww_cusres_dq)
    df_gts_qty = get_gts('MX', df_gts_qty)
    df_gts_dq = get_gts('MX', df_gts_dq)

    df_master = get_cusres_qty(df_ww_cusres_qty, df_gts_qty, 'MX')
    df_final = separate_data_with_same_part_number(df_ww_cusres_dq, df_gts_dq)
    df_final = compare(df_final, df_duty)


    df_final['SHIP_TO_COUNTRY'] = 'MX'
    ...
    with pd.ExcelWriter(f'../../PBI/Kill Random Review/Import Declaration Reconciliation Report Mexico.xlsx') as writer:
    ...
    with pd.ExcelWriter(f'output/Import Declaration Reconciliation Report Mexico.xlsx') as writer:
        df_final.to_excel(writer, index=False, sheet_name='CUSRES DQ')
        df_master.to_excel(writer, index=False, sheet_name='CUSRES QTY')
    ...
    file_path_report = pathlib.Path('output/Import Declaration Reconciliation Report Mexico.xlsx')
    file_absolute_report = str(file_path_report.absolute())
    file_path_img = pathlib.Path('input/LAR Import Declaration Reconciliation - (MX).png')
