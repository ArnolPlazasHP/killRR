...
LIST_HTS_SPECIAL = ['84439930', '84439990', '84717000', '84718000', '85044000', '85285230', '90138000']
...
def get_db_additional(country, country_spacial):
    ...
    df_duty_spacial = pd.read_excel('../../PBI/Kill Random Review/Duty Tariff MCA.xlsx', sheet_name=country_spacial)
    df_duty_spacial = df_duty_spacial.convert_dtypes()
    df_duty_spacial['HTS'] = df_duty_spacial['HTS'].astype('string')
    df_duty_spacial['key'] = df_duty_spacial['HTS'] + df_duty_spacial['TT5C Shipref']
    df_duty_spacial = df_duty_spacial[['key', 'DUTY', 'VAT']]
    df_duty_spacial.dropna(inplace=True)

    return df_duty, df_duty_spacial


def compare(df_final, df_duty, df_duty_spacial):
    ...
    df_final_special = df_final[df_final['HTS'].isin(LIST_HTS_SPECIAL)]
    df_final_no_special = df_final[~df_final['HTS'].isin(LIST_HTS_SPECIAL)]

    df_final_special['key'] = df_final_special['HTS'] + df_final_special['TT5C Shipref']
    df_final_special = df_final_special.merge(df_duty_spacial, on='key', how='left')
    df_final_special.drop(columns=['key'], inplace=True)
    df_final_no_special = df_final_no_special.merge(df_duty, on='HTS', how='left')

    df_final = pd.concat([df_final_special, df_final_no_special])
    ...
    df_final['HTS Class'] = df_final['HTS Class'].apply(lambda x : x[:8])

    df_final_null = df_final[df_final['LINE_AMOUNT'] == 'Blank']
    df_final_not_null = df_final[df_final['LINE_AMOUNT'] != 'Blank']
    df_final_not_null = update_country_destination(df_final_not_null)
    df_final = pd.concat([df_final_null, df_final_not_null])
    ...
    df_final['COMPARE_DUTY'] = df_final.apply(lambda x: compare_columns2(x['VALIDATION DUTY'], x['Duty Charge Amount'], 'COMPARE_DUTY') if x['VALIDATION DUTY'] != 'Blank' and x['Duty Charge Amount'] != 'Blank' else compare_columns(x['VALIDATION DUTY'], x['Duty Charge Amount']), axis=1)
    df_final['COMPARE_LINE_AMOUNT'] = df_final.apply(lambda x: compare_columns2(x['LINE_AMOUNT'], x['Item Total Amount'], 'COMPARE_LINE_AMOUNT'), axis=1)
    df_final['COMPARE_LINE_QTY'] = df_final.apply(lambda x: compare_columns2(x['LINE_QTY'], x['Qty in Doc. UoM'], 'COMPARE_LINE_QTY'), axis=1)
    df_final['COMPARE_VAT_RATE'] = df_final.apply(lambda x: compare_columns(x['VAT'], x['Import VAT Tax Rate']), axis=1)
    df_final['COMPARE_VAT'] = df_final.apply(lambda x: compare_columns2(x['VALIDATION VAT'], x['Import VAT Tax amount'],'COMPARE_VAT') if x['VALIDATION VAT'] != 'Blank' and x['Import VAT Tax amount'] != 'Blank' else compare_columns(x['VALIDATION VAT'], x['Import VAT Tax amount']) , axis=1)
    df_final['COMPARE_UNIT_PRICE'] = df_final.apply(lambda x: compare_columns2(x['UNIT_PRICE'], x['Unit Price'], 'COMPARE_UNIT_PRICE'), axis=1)
    df_final['COMPARE_TOTAL_INV_AMT'] = df_final.apply(lambda x: compare_columns2(x['TOTAL_INV_AMT'], x['Invoice total value'], 'COMPARE_TOTAL_INV_AMT'), axis=1) 
...
def compare_columns2(c1, c2, c):
    try:
        c1  = float(c1)
        c2  = float(c2)
    except:
        pass
    if c1 == 0 and c2 != 0 and c in ['COMPARE_TOTAL_INV_AMT', 'COMPARE_UNIT_PRICE', 'COMPARE_LINE_AMOUNT']:
        return 'Zero in GTS'
    elif c1 != 'Blank' and c2 == 'Blank':
        return 'Blanks in Cusres'
    elif c1 == 'Blank' and c2 != 'Blank':
        ...
        return 'Equal'
    elif  c in ['COMPARE_DUTY', 'COMPARE_VAT', 'COMPARE_LINE_AMOUNT', 'COMPARE_UNIT_PRICE']:
        if abs(c2-c1)/((c1 + c2)/2) <= 0.03:
            return 'Equal'
        else:
            return 'Different'
    elif abs(c1 - c2) < 10:
...
def send_email(list_criterio, list_qty_equal, list_qty_different, list_qty_blanks_cusres, week, range_date, total_rows, file_absolute_report, file_absolute_img):
    ...
    message.To = "pcampos@browne.cl; mgoffredo2@browne.cl; luis.henriquez@hp.com; karen.salviz@hp.com; rafael.patino@hp.com; karla.botello.tirado@hp.com; bfernandez@browne.cl"
    #message.To = "arnol.plazas@hp.com"
...
def get_cusres_qty(df_ww_cusres, df_gts, country):
    ...
    df_dmr['DESTINATION COUNTRY'] = df_dmr['DESTINATION COUNTRY'].str.replace('Chile', country)
...
def update_country_destination(df_final):
    df_group_entry_number_country = df_final.groupby(['Entry Number', 'COO'])['LINE_AMOUNT'].sum().reset_index()
    df_group_entry_number = df_final.groupby('Entry Number')['LINE_AMOUNT'].sum().reset_index()
    df_group_entry_number.rename(columns={'LINE_AMOUNT': 'LINE_AMOUNT_TOTAL'}, inplace=True)
    df_group_entry_number_country = df_group_entry_number_country.merge(df_group_entry_number, on= 'Entry Number', how='inner')
    df_group_entry_number_country['PERCENTAGE'] = df_group_entry_number_country.apply(lambda x: get_percentage(x['LINE_AMOUNT'], x['LINE_AMOUNT_TOTAL']), axis=1)
    df_group_entry_number_country = df_group_entry_number_country[df_group_entry_number_country['PERCENTAGE'] > .5]
    df_group_entry_number_country.rename(columns={'COO': 'COO_GREATHER'}, inplace=True)
    df_final = df_final.merge(df_group_entry_number_country[['Entry Number', 'COO_GREATHER']], on= 'Entry Number', how='left')
    df_final.loc[~df_final['COO_GREATHER'].isnull(), 'COO'] = df_final['COO_GREATHER']
    df_final.drop(columns=['COO_GREATHER'], inplace=True)
    return df_final
...
def run():
    ...
    df_duty, df_duty_spacial = get_db_additional('CL', 'CL SPECIAL INSTRUCTION')

    df_ww_cusres_qty = get_cusres('CL', df_ww_cusres_qty)
    df_ww_cusres_dq = get_cusres('CL', df_ww_cusres_dq)
    df_gts_qty = get_gts('CL', df_gts_qty)
    df_gts_dq = get_gts('CL', df_gts_dq)

    df_master = get_cusres_qty(df_ww_cusres_qty, df_gts_qty, 'CL')
    df_final = separate_data_with_same_part_number(df_ww_cusres_dq, df_gts_dq)
    df_final = compare(df_final, df_duty, df_duty_spacial)


    df_final['SHIP_TO_COUNTRY'] = 'CL'
    ...
    with pd.ExcelWriter(f'../../PBI/Kill Random Review/Import Declaration Reconciliation Report Chile.xlsx') as writer:
    ...
    with pd.ExcelWriter(f'output/Import Declaration Reconciliation Report Chile.xlsx') as writer:
        df_final.to_excel(writer, index=False, sheet_name='CUSRES DQ')
        df_master.to_excel(writer, index=False, sheet_name='CUSRES QTY')
    ...
    file_path_report = pathlib.Path('output/Import Declaration Reconciliation Report Chile.xlsx')
    file_absolute_report = str(file_path_report.absolute())
    file_path_img = pathlib.Path('input/LAR Import Declaration Reconciliation - (CL).png')
