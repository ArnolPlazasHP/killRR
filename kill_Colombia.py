...
def compare(df_final, df_duty):
    ...
    df_final.loc[:,['CURRENCY', 'HTS assigned', 'HTS', 'HTS Class']].fillna('Blank', inplace=True)
    ...
    # HTS
    df_final['HTS assigned'] = df_final['HTS assigned'].apply(lambda x : x[:x.rfind('.')] if x.rfind('.') != -1 else x)
    df_final['HTS'] = df_final['HTS'].apply(lambda x : x[:x.rfind('.')] if x.rfind('.') != -1 else x)
    df_final['HTS Class'] = df_final['HTS Class'].apply(lambda x : x[:x.rfind('.')] if x.rfind('.') != -1 else x)
    df_duty['HTS'] = df_duty['HTS'].apply(lambda x : x[:x.rfind('.')] if x.rfind('.') != -1 else x)
    ...
    df_final['VALIDATION DUTY'] = ((df_final['LINE_AMOUNT'] + df_final['Harbor Maintence Fee (Amount)']) * df_final['DUTY']) * df_final['Exchange Rate (KF)']
    df_final['VALIDATION VAT'] = (df_final['VALIDATION DUTY'] + (df_final['LINE_AMOUNT'] * df_final['Exchange Rate (KF)']) + df_final['Harbor Maintence Fee (Amount)']) * df_final['VAT']
    ...
    df_final['HTS'].fillna('Blank', inplace=True)


    df_final['Exchange'] = df_final.apply(lambda x: 0 if  pd.isna(x['UNIT_PRICE']) | pd.isna(x['Exchange Rate (KF)']) else x['UNIT_PRICE'] * x['Exchange Rate (KF)'], axis=1)
    df_final['VALIDATION VAT'] = df_final.apply(lambda x: 0 if (x['HTS'] == '8471490000' or x['HTS'] =='8471300000') and (x['Exchange']< 1900200) else x['VALIDATION VAT'], axis=1)
    df_final['VAT'] = df_final.apply(lambda x: 0 if (x['HTS'] == '8471490000' or x['HTS'] =='8471300000') and (x['Exchange']< 1900200)    else x['VAT'], axis=1)
    df_final.drop(columns=['Exchange'], inplace=True)
    ...
    df_final['COMPARE_DUTY'] = df_final.apply(lambda x: compare_columns3(x['VALIDATION DUTY'], x['Duty Charge Amount']) if x['VALIDATION DUTY'] != 'Blank' and x['Duty Charge Amount'] != 'Blank' else compare_columns(x['VALIDATION DUTY'], x['Duty Charge Amount']), axis=1)
    ...
    df_final['COMPARE_VAT'] = df_final.apply(lambda x: compare_columns3(x['VALIDATION VAT'], x['Import VAT Tax amount']) if x['VALIDATION VAT'] != 'Blank' and x['Import VAT Tax amount'] != 'Blank' else compare_columns(x['VALIDATION VAT'], x['Import VAT Tax amount']) , axis=1)
...
def compare_columns3(c1, c2):
    ...
    if c1 != 'Blank' and c2 == 'Blank':
        ...
        return 'Blanks in GTS'
    if c1 == c2 or abs(c2-c1)/((c1 + c2)/2) <= 0.03:
        return 'Equal'
    else:
        return 'Different'
...
def send_email(list_criterio, list_qty_equal, list_qty_different, list_qty_blanks_cusres, week, range_date, total_rows, file_absolute_report, file_absolute_img):
    ...
    message.To = "manulyi.romero@repremundo.com.co; luis.henriquez@hp.com; leidy.mendivelso@repremundo.com.co; sergio.corredor@repremundo.com.co; karen.salviz@hp.com; alejandra.fernandez@repremundo.com.co; karen.gomez@repremundo.com.co; loren.calderon@repremundo.com.co; rafael.patino@hp.com; karla.botello.tirado@hp.com; fabian.parra@repremundo.com.co"
...
def get_cusres_qty(df_ww_cusres, df_gts, country):
    ...
    df_dmr['DESTINATION COUNTRY'] = df_dmr['DESTINATION COUNTRY'].str.replace('Colombia', country)
...
def run():
    ...
    df_duty = get_db_additional('CO')

    df_ww_cusres_qty = get_cusres('CO', df_ww_cusres_qty)
    df_ww_cusres_dq = get_cusres('CO', df_ww_cusres_dq)
    df_gts_qty = get_gts('CO', df_gts_qty)
    df_gts_dq = get_gts('CO', df_gts_dq)

    df_master = get_cusres_qty(df_ww_cusres_qty, df_gts_qty, 'CO')
    ...
    with pd.ExcelWriter(f'../../PBI/Kill Random Review/Import Declaration Reconciliation Report Colombia.xlsx') as writer:
    ...
    with pd.ExcelWriter(f'output/Import Declaration Reconciliation Report Colombia.xlsx') as writer:
        df_final.to_excel(writer, index=False, sheet_name='CUSRES DQ')
        df_master.to_excel(writer, index=False, sheet_name='CUSRES QTY')
    ...
    file_path_report = pathlib.Path('output/Import Declaration Reconciliation Report Colombia.xlsx')
    file_absolute_report = str(file_path_report.absolute())
    file_path_img = pathlib.Path('input/LAR Import Declaration Reconciliation - (CO).png')
