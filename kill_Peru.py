...
def compare(df_final, df_duty):
    ...
    df_final['COMPARE_DUTY'] = df_final.apply(lambda x: compare_columns(x['VALIDATION DUTY'], x['Duty Charge Amount']) if x['VALIDATION DUTY'] != 'Blank' and x['Duty Charge Amount'] != 'Blank' else compare_columns(x['VALIDATION DUTY'], x['Duty Charge Amount']), axis=1)
    ...
    df_final['COMPARE_VAT'] = df_final.apply(lambda x: compare_columns(x['VALIDATION VAT'], x['Import VAT Tax amount']) if x['VALIDATION VAT'] != 'Blank' and x['Import VAT Tax amount'] != 'Blank' else compare_columns(x['VALIDATION VAT'], x['Import VAT Tax amount']) , axis=1)
...
def send_email(list_criterio, list_qty_equal, list_qty_different, list_qty_blanks_cusres, week, range_date, total_rows, file_absolute_report, file_absolute_img):
    ...
    message.To = "Luis.Iwasaki@dhl.com; karen.salviz@hp.com; jose.bustillos@dhl.com; rendy.marquez@dhl.com; Cynthia.Carrasco@dhl.com; luis.henriquez@hp.com; rafael.patino@hp.com; karla.botello.tirado@hp.com"
...
def get_cusres_qty(df_ww_cusres, df_gts, country):
    ...
    df_dmr['DESTINATION COUNTRY'] = df_dmr['DESTINATION COUNTRY'].str.replace('Peru', country)
...
def run():
    ...
    df_duty = get_db_additional('PE')

    df_ww_cusres_qty = get_cusres('PER', df_ww_cusres_qty)
    df_ww_cusres_dq = get_cusres('PER', df_ww_cusres_dq)
    df_gts_qty = get_gts('PE', df_gts_qty)
    df_gts_dq = get_gts('PE', df_gts_dq)

    df_master = get_cusres_qty(df_ww_cusres_qty, df_gts_qty, 'PE')
    df_final = separate_data_with_same_part_number(df_ww_cusres_dq, df_gts_dq)
    df_final = compare(df_final, df_duty)


    df_final['SHIP_TO_COUNTRY'] = 'PE'
    ...
    with pd.ExcelWriter(f'../../PBI/Kill Random Review/Import Declaration Reconciliation Report Peru.xlsx') as writer:
    ...
    with pd.ExcelWriter(f'output/Import Declaration Reconciliation Report Peru.xlsx') as writer:
        df_final.to_excel(writer, index=False, sheet_name='CUSRES DQ')
        df_master.to_excel(writer, index=False, sheet_name='CUSRES QTY')
    ...
    file_path_report = pathlib.Path('output/Import Declaration Reconciliation Report Peru.xlsx')
    file_absolute_report = str(file_path_report.absolute())
    file_path_img = pathlib.Path('input/LAR Import Declaration Reconciliation - (PE).png')
